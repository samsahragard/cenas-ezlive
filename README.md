...
5. `kitchen_engine` runs per-package rules: `rules_fajitas` (mixed: 2.5 oz chicken + 2.5 oz beef per person; solo: 5 oz/pp), `rules_brochette` (2 packs/pp), `rules_veggie` (6 oz/pp), `rules_salads` (cobb + fajita-and-salad with all the lettuce/avocado/tomato/cuke/cheese/bacon/egg/olives portions), `party_sides` (onions 1.5 oz [1.0 over 30, 0.7 over 50], pico 1.5 oz [1.0 over 30], sour cream 1.5 oz [1.0 over 30, 0.8 over 50], guac 1.5 oz, rice/beans 3.8 oz [3.5 over 30, 3.0 over 50], chips 3.0 oz [2.5 over 30, 2.3 over 50], red/green sauce 1.5 oz), tortilla packets (2.5/2 packets/pp with proper half/half splitting), and tableware utensil aggregation across all trays + dessert tongs + cobb prep components
6. `dispatch_planner` pairs same-store same-day orders (60 min kitchen-ready buffer, departure at window start minus drive time, 20 min multi-stop service buffer), scores all feasible pairs by `(total_late, total_drive)`, greedy-assigns best-first, solo fallback. Drive minutes from Google Maps Distance Matrix.
