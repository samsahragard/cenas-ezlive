...
@appdb_export_bp.route("/cron/fresh-food-order-cleanup", methods=["POST"])
def fresh_food_order_cleanup():
    expected = _expected_token()
    got = _extract_token() or ""
    if not expected or not hmac.compare_digest(got, expected):
        abort(403)
    ...
    try:
        keep_order_id = int(body.get("keep_order_id"))
    except (TypeError, ValueError):
        return jsonify({"ok": False, "error": "keep_order_id required"}), 400
    if keep_order_id != 13:
        return jsonify({"ok": False, "error": "refusing unexpected keep_order_id"}), 400

    if engine is None or engine.url.get_backend_name() != "sqlite":
        return jsonify({"ok": False, "error": "cleanup supports sqlite only"}), 501
    src = engine.url.database
    if not src or not os.path.exists(src):
        return jsonify({"ok": False, "error": "prod db file not found"}), 500

    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    backup_path = os.path.join(
        os.path.dirname(src),
        f"fresh_food_order_cleanup_before_keep_{keep_order_id}_{ts}.json",
    )
    ...
    try:
        ...
        keep = con.execute(
            "SELECT id FROM fresh_food_order WHERE id = ?",
            (keep_order_id,),
        ).fetchone()
        if keep is None:
            con.rollback()
            return jsonify({"ok": False, "error": "keep order not found"}), 404

        orders = [
            dict(r)
            for r in con.execute("SELECT * FROM fresh_food_order ORDER BY id")
        ]
        lines = [
            dict(r)
            for r in con.execute(
                "SELECT * FROM fresh_food_order_line ORDER BY order_id, id"
            )
        ]
        tmp_backup = backup_path + ".tmp"
        with open(tmp_backup, "w", encoding="utf-8") as f:
            json.dump({"orders": orders, "lines": lines}, f, default=str)
        os.replace(tmp_backup, backup_path)

        deleted_lines = con.execute(
            "DELETE FROM fresh_food_order_line WHERE order_id <> ?",
            (keep_order_id,),
        ).rowcount
        deleted_orders = con.execute(
            "DELETE FROM fresh_food_order WHERE id <> ?",
            (keep_order_id,),
        ).rowcount

        positive_count = con.execute(
            """
            SELECT COUNT(*) FROM fresh_food_order_line
            WHERE order_id = ? AND or_qty IS NOT NULL AND or_qty > 0
            """,
            (keep_order_id,),
        ).fetchone()[0]
        missing_sent = con.execute(
            """
            SELECT COUNT(*) FROM fresh_food_order_line
            WHERE order_id = ? AND or_qty IS NOT NULL AND or_qty > 0
              AND sent_qty IS NULL
            """,
            (keep_order_id,),
        ).fetchone()[0]
        if positive_count and missing_sent == 0:
            con.execute(
                "UPDATE fresh_food_order SET status = 'completed' WHERE id = ?",
                (keep_order_id,),
            )

        remaining_orders = [
            r[0]
            for r in con.execute("SELECT id FROM fresh_food_order ORDER BY id")
        ]
        con.commit()
    except Exception as e:  # noqa: BLE001 - token-gated operator route
        con.rollback()
        return jsonify({
            "ok": False,
