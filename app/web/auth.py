...
EXEMPT_PREFIXES = (
    ...
    "/employee/time-off",        # Schedules V2 B7: the employee_time_off_page route self-guards on session["employee_id"] and 302s to /employee/login (not the keypad). One prefix also covers ckai's /employee/time-off/list + /request + /<id> endpoints (each own employee_id guard). NOTE: ckai's B7 backend EXEMPTs this same prefix too - dedupe one at merge (harmless duplicate).
