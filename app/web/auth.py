...
EXEMPT_PREFIXES = (
    ...
    "/employee/tables",          # Employee own table/check timeline page + JSON; self-guards employee_id and never exposes cross-employee Toast data.
    ...
    "/employee/roster",          # Employee weekly roster page + data; self-guards employee_id and only exposes the logged-in employee's store roster.
    "/employee/sports",          # Employee Sports tab (Sam 2026-06-13): PAGE GET /employee/sports (HTML; self-guards employee_id -> 302 /employee/login) + DATA /employee/sports/data.json (self-guards -> 401 JSON). Same pattern as /employee/my-schedule.
