...
# ============================================================
# TEMP 2026-06-27: exact Copperfield Sling draft import
# ============================================================

@cena_bp.route("/sam/cena/run-cf-schedule-import-20260628", methods=["POST"])
def cena_run_cf_schedule_import_20260628():
    gate = _require_gateway_token()
    if gate is not None:
        return gate
    ...
    expected_sha = "381db1692101c6867dad134b324a2bb9ae502bc2286eb0461a559accc6bb0fcc"
    ...
    if week_start.isoformat() != "2026-06-28":
        return jsonify({"ok": False, "error": "only 2026-06-28 week is allowed"}), 400
    if (payload.get("target_store") or "").strip().lower() != "copperfield":
        return jsonify({"ok": False, "error": "only copperfield target_store is allowed"}), 400
    if not bool(payload.get("replace_existing")):
        return jsonify({"ok": False, "error": "replace_existing must be true"}), 400
    if not isinstance(records, list) or not records or len(records) > 1000:
        return jsonify({"ok": False, "error": "records[] required, max 1000"}), 400
    ...
    actor_id = None
    try:
        actor_id = int(os.getenv("SAM_CHAT_USER_ID") or 0) or None
    except (TypeError, ValueError):
        actor_id = None
    db = SessionLocal()
    try:
        ...
        summary["payload_sha256"] = actual_sha
        return jsonify(summary), (200 if summary.get("ok") else 409)
    except Exception as exc:  # noqa: BLE001
        db.rollback()
        logger.exception("cena: cf schedule import crashed")
