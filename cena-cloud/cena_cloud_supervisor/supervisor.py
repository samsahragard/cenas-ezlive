"""Vendored CENA supervisor lane for the cena-cloud service.

This is a near-verbatim port of the CK-local runtime answer logic from
``cenas-kitchen-runtime/scripts/assistant_ck_runtime.py``. ONLY the import
paths were rewritten so it runs inside the cena-cloud package with no ``app``
or ``scripts`` package present:

  * the L3 engine  -> ``cena_engine.cena_sql_orchestrator.answer_question``
  * routing/safety -> the vendored ``cena_cloud_supervisor.assistant_*`` siblings
  * conversations  -> ``cena_cloud_supervisor.assistant_conversations``
  * review queue   -> ``cena_cloud_supervisor.assistant_review_ck_receiver``

The supervisor wrapper, per-turn grading, CK/DEV rescue, the yes/no ladder,
the DEV intro, the 48h conversation store, and the observe-mode state machine
are all preserved unchanged. The HTTP server / auth / main() from the runtime
are intentionally omitted: cena_cloud.py owns HTTP and calls ``_answer(payload)``
...
Environment (all paths ENV-overridable; cloud points them at /var/data):
  ...
  ASSISTANT_REVIEW_DB                    review-queue sqlite
  CENA_HUB_URL                           LAN hub (unreachable on cloud; fire-and-forget)
"""
...
# cena-cloud root (the dir holding cena_engine/) must be importable so the
# vendored L3 engine resolves regardless of CWD.
_REPO_ROOT = Path(__file__).resolve().parents[1]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from . import assistant_review_ck_receiver as review_receiver
...
from .assistant_context import load_assistant_context as _load_assistant_context
from .assistant_tool_inventory import iter_readonly_operational_tool_specs
...
def _diag_event(event: str, **fields: object) -> None:
    try:
        path = Path(os.getenv("ASSISTANT_RUNTIME_DIAG_LOG") or r"C:\Users\sam\cena-ai-assistant\assistant_runtime_diag.log")
...
def _token_fingerprint(value: str) -> dict[str, object]:
    if not value:
...
from . import assistant_conversations as _conv

_conv.init_db()


_BUILD = os.getenv("CENA_CLOUD_BUILD") or "cena-cloud"
...
# NOTE (cloud port): these four helpers build LIVE Toast API payloads and only
# run on the CK-local box (app.services.toast_* + local Toast creds). On the
# cloud, Toast tool payloads always arrive via the relay's `tool_data`; these
# builders are never the source of truth. The only reachable call site
# (_toast_webhook_activity_payload in _approved_tool_answer) is wrapped in
# try/except and falls through to None on failure, so raising here is identical
# in behavior to a failed live Toast call on CK.
def _toast_sales_summary_payload(period: str) -> dict:
    raise RuntimeError("toast live payloads are CK-local only; cloud uses tool_data")
...
def _answer_core(payload: dict) -> tuple[dict, int]:
...
# --- C.E.N.A. developer (CK) supervisor lane -------------------------------
# Wraps the legacy answer path with: 48h conversations, per-turn grading,
# CK rescue answers, and the cena_active / ck_engaged observe-mode state
# machine. Any failure in this layer falls back to the legacy path.

def _ck_persona_prompt(principal: dict) -> str:
    return (
        "You are the C.E.N.A. developer (CK), the engineer who supervises the "
        "Cenas Kitchen in-app assistant and steps in when C.E.N.A. cannot answer "
        "properly. Follow the exact same policy rules as C.E.N.A.: answer only "
        "within the current user's role and permissions; never reveal secrets, "
        "passcodes, tokens, customer PII, unauthorized payroll, raw peer pay, "
        "sales internals, GUIDs, or cross-store data. If the data needed is not "
        "available, say so plainly and say what IS available instead. Be direct, "
        "friendly, and brief.\n\n" + _session_prompt(principal)
    )


def _investigation_answer_for_ck(question: str, principal: dict) -> dict | None:
    """L3 without the keyword gate - the supervisor decides when to investigate."""
    try:
        from cena_engine.cena_sql_orchestrator import answer_question
    except Exception:  # noqa: BLE001
        return None
    try:
        ...
        log.exception("ck supervisor: L3 rescue failed")
        return None
    if not isinstance(res, dict) or not res.get("ok") or not str(res.get("answer", "")).strip():
        return None
    return res
...
def _ck_rescue_answer(question: str, resolved_question: str, principal: dict,
    ...
    if _conv.is_action_request(question):
        ref = ""
        try:
            row = _queue_for_review(
                question, principal,
                "sensitive_or_operational_question_needs_approved_tool",
                None, "ck_supervisor",
            )
            if row and row.get("ck_question_id"):
                ref = f" (ref {row['ck_question_id']})"
        except Exception:  # noqa: BLE001
            ref = ""
        text = (
            "I can't take actions from this chat - I'm read-only here (no "
            "scheduling, comping, emailing, or staffing changes). I've saved "
            f"your request for Sam{ref}. If it's urgent, reach him directly."
        )
        return text, {"ck_mode": "action_decline"}

    if _conv.looks_dataish(resolved_question) and bool((principal or {}).get("can_ask_operational")):
        res = _investigation_answer_for_ck(resolved_question, principal)
        if res is not None:
            return str(res.get("answer")), {
                "ck_mode": "l3_rescue",
                "confidence": res.get("confidence"),
                "confidence_reason": res.get("confidence_reason"),
                "show_work": res.get("show_work"),
            }

    history = ""
    try:
        hits = _conv.search_history(principal, resolved_question, limit=3)
        if hits:
            lines = [f"- [{h['ts']}] {h['sender']}: {str(h['text'])[:240]}" for h in hits]
            history = "\nRelevant lines from this user's previous chats:\n" + "\n".join(lines)
    except Exception:  # noqa: BLE001
        history = ""
    prompt = (
        _ck_persona_prompt(principal)
        + f"\n\nThe user asked:\n{resolved_question}\n"
        + f"\nC.E.N.A.'s answer was graded as needing help ({grade_reason})"
        + (f" and read:\n{cena_answer[:800]}\n" if cena_answer else ".\n")
        + history
        + "\nWrite the correct, helpful answer now. If the question is ambiguous, "
          "give the most useful reading and ask ONE short clarifying question at the end."
    )
    text = None
    try:
    ...
    if not text:
        text = (
            "I couldn't pull the right data for this just now - I've flagged it "
            "and will follow up. Anything else I can check?"
        )
    return text, {"ck_mode": "gemini_rescue"}


def _ck_clarify(question: str, principal: dict, conversation_id: int) -> str:
    """User said the CK answer didn't help: ask fine-tuning questions."""
    recent = ""
    try:
        msgs = _conv.get_thread(conversation_id, limit=200)[-6:]
        recent = "\n".join(f"{m['sender']}: {str(m['text'])[:300]}" for m in msgs)
    except Exception:  # noqa: BLE001
        recent = ""
    prompt = (
        _ck_persona_prompt(principal)
        + "\n\nThe user said the previous answer did NOT answer their question. "
          "Recent conversation:\n" + recent
        + "\n\nApologize in one short sentence, then ask one or two SHORT, specific "
          "questions that pin down exactly what they need (time range? store? metric?). "
          "If you can already tell what they meant, give the corrected answer too."
    )
    text = None
    try:
        text, _model = _gemini_generate(prompt)
        ...
        text = None
    return text or (
        "Sorry about that - let me narrow it down. Which store did you mean, and "
        "what date range are you asking about?"
    )


def _ck_engaged_turn(conv: dict, question: str, resolved_question: str,
                     principal: dict) -> tuple[dict, int] | None:
    """Handle a turn while CK owns the thread (CENA observes, never replies).
    Returns None when the engagement should release and the normal CENA path
    should process this turn instead."""
    released = False
    if _conv.is_affirmation(question):
        _conv.set_state(conv["id"], "cena_active")
        ck_text = _conv.RELEASE_TEXT
        mode = "release_affirmed"
        released = True
    elif _conv.is_negative(question):
        if _conv.last_ckdev_route(conv["id"]) == "ck_clarify":
            # Clarifying didn't land either: save it for Sam and hand back.
            original = _conv.last_substantive_user_question(conv["id"]) or question
            ref = ""
            try:
                row = _queue_for_review(
                    original, principal,
                ...
                ref = ""
            ck_text = (
                "Thanks for bearing with me - I couldn't nail this one down, so "
                f"I've saved it for Sam to review personally{ref}. He'll follow "
                "up, and I'll make sure C.E.N.A. learns it. Anything else I can check?"
            )
            mode = "queued_for_sam"
            _conv.set_state(conv["id"], "cena_active")
            released = True
            _conv.hub_post(
                f"[supervise] conv#{conv['id']} escalated to Sam review after "
                f"two NOs: {original[:140]}",
                author="cena-supervisor",
            )
        else:
            ck_text = _ck_clarify(question, principal, conv["id"])
            mode = "retry_clarify"
    else:
        ck_text, meta = _ck_rescue_answer(question, resolved_question, principal,
                                          "ck_engaged_followup", "")
        ck_text = ck_text + "\n\n" + _conv.FOLLOWUP_TEXT
        mode = meta.get("ck_mode", "engaged_answer")

    route_label = {"retry_clarify": "ck_clarify", "queued_for_sam": "ck_queued"}.get(mode, "ck_engaged")
    _conv.add_message(conv["id"], "ck-dev", ck_text, route=route_label, meta={"mode": mode})
    _conv.log_turn({
        "conversation_id": conv["id"],
        "principal_key": _conv.principal_key(principal),
    ...
    body = {
        ...
        "ck_answer": ck_text,
        "queued": False,
        "route_path": "ck_engaged",
        "routed_tool_id": None,
        "conversation_id": conv["id"],
        "ck_engaged": not released,
        "storage": "ck_runtime",
    }
    return body, 200
...
def _answer(payload: dict) -> tuple[dict, int]:
    """Dual-responder wrapper: conversations + grading + CK rescue around the
    legacy answer path. Fails open to _answer_core on any internal error."""
    try:
        question = str(payload.get("question") or "").strip()[:_MAX_QUESTION_CHARS]
        principal = payload.get("principal") or {}
        previous_question = str(payload.get("previous_question") or "").strip()[:_MAX_QUESTION_CHARS]
        if not question:
            return _answer_core(payload)

        conv, new_chat = _conv.get_active_conversation(principal)
        state = _conv.effective_state(conv)
        silence_ok = _conv.silence_resolved(conv["id"])
        prev_user_q = _conv.last_user_question(conv["id"]) or ""
        last_sender_val = _conv.last_sender(conv["id"])
        first_cena_answer = _conv.cena_answer_count(conv["id"]) == 0
        ...
        if state == "ck_engaged" and _DEV_PERSONA_ENABLED:
            if silence_ok and not _conv.is_negative(question) and not _conv.is_affirmation(question):
                ...
                engaged = _ck_engaged_turn(conv, question, resolved_question, principal)
                if engaged is not None:
                    return engaged

        # Quick yes/no right after a CENA/DEV answer ("Did I answer your question?").
        # Last sender can be ck-dev too (e.g. the DEV intro lands after CENA's
        # first answer) - feedback still refers to the answer above it.
        if last_sender_val in ("cena", "ck-dev") and _conv.is_affirmation(question):
            _conv.add_message(conv["id"], "cena", _conv.ACK_TEXT, route="ack")
            _conv.log_turn({
                "conversation_id": conv["id"],
                "principal_key": _conv.principal_key(principal),
                "role": (principal or {}).get("role"),
            ...
            return {
                "ok": True, "answer": _conv.ACK_TEXT, "cena_answer": _conv.ACK_TEXT,
                "queued": False, "route_path": "ack", "routed_tool_id": None,
                "conversation_id": conv["id"], "storage": "ck_runtime",
            }, 200
        if last_sender_val in ("cena", "ck-dev") and _conv.is_negative(question):
            # User says CENA missed: DEV immediately attempts the ORIGINAL
            # question (plus any extra detail the user packed into the "no").
            extra = _conv.strip_negation(question)
            if extra and prev_user_q:
            ...
            _conv.set_state(conv["id"], "ck_engaged")
            intro_prefix = ""
            if not _conv.dev_intro_shown(conv["id"]):
                intro_prefix = _conv.INTRO_TEXT + "\n\n"
            ck_text = intro_prefix + ck_answer + "\n\n" + _conv.FOLLOWUP_TEXT
            _conv.add_message(conv["id"], "ck-dev", ck_text, route="ck_rescue", meta=ck_meta)
            _conv.log_turn({
                "conversation_id": conv["id"],
                "principal_key": _conv.principal_key(principal),
                "role": (principal or {}).get("role"),
                ...
                "route": "ck_rescue",
                "grade": "user_no",
                "grade_reason": "user_said_no",
                "ck_answer": ck_text[:2000],
                "ck_mode": ck_meta.get("ck_mode"),
            })
            _conv.hub_post(
                f"[supervise] conv#{conv['id']} role={(principal or {}).get('role')} "
                f"user said NO -> DEV engaged ({ck_meta.get('ck_mode')}) on: {target[:140]}",
                author="cena-supervisor",
            )
            return {
                "ok": True, "answer": "DEV: " + ck_text, "cena_answer": None,
                "ck_answer": ck_text, "queued": False, "route_path": "ck_rescue",
                "routed_tool_id": None, "conversation_id": conv["id"],
                "ck_engaged": True, "storage": "ck_runtime",
            }, 200

        body, status = _answer_core(payload)
        grade, grade_reason = _conv.grade_answer(question, body, status, principal)
        cena_answer = str(body.get("answer") or "")
        # CENA always closes with the short follow-up question (not on queued).
        if cena_answer and status == 200 and body.get("ok") and not body.get("queued"):
            cena_answer = cena_answer.rstrip() + "\n\n" + _conv.CENA_FOLLOWUP
        ...
        if cena_answer:
            _conv.add_message(
                conv["id"], "cena", cena_answer,
                route=str(body.get("route_path") or ""),
                confidence=str(body.get("confidence") or "") or None,
                grade=grade,
            )
        ...
        if ck_answer:
            _conv.add_message(
                conv["id"], "ck-dev", ck_answer + "\n\n" + _conv.FOLLOWUP_TEXT,
                route="ck_rescue", grade=grade, meta=ck_meta,
            )

        body = dict(body)
        body["conversation_id"] = conv["id"]
        body["new_chat"] = bool(new_chat)
        body["grade"] = grade
        body["grade_reason"] = grade_reason
        body["cena_answer"] = cena_answer or None
        if cena_answer:
            body["answer"] = cena_answer
        if intro:
            body["ck_intro"] = intro
        if ck_answer:
            body["ck_answer"] = ck_answer
            body["ck_followup"] = _conv.FOLLOWUP_TEXT
            body["ck_engaged"] = True
        dev_parts = []
        if intro:
            dev_parts.append(intro)
        if ck_answer:
            dev_parts.append(ck_answer + "\n\n" + _conv.FOLLOWUP_TEXT)
        if dev_parts:
            legacy = (cena_answer + "\n\n") if cena_answer else ""
            body["answer"] = legacy + "DEV: " + "\n\n".join(dev_parts)

        _conv.log_turn({
            "conversation_id": conv["id"],
            "principal_key": _conv.principal_key(principal),
            "role": (principal or {}).get("role"),
            "stores": (principal or {}).get("store_slugs"),
            "question": question,
            "route": str(body.get("route_path") or ""),
            ...
            "cena_answer": cena_answer[:2000],
            "ck_answer": (ck_answer or "")[:2000] or None,
            "ck_mode": ck_meta.get("ck_mode"),
            "http_status": status,
        })
        if _DEV_PERSONA_ENABLED and grade in ("medium", "low"):
            _conv.hub_post(
                ...
                f"({ck_meta.get('ck_mode')})",
                author="cena-supervisor",
            )
        return body, status
    except Exception:  # noqa: BLE001
        log.exception("assistant runtime: supervisor wrapper failed; using core path")
        return _answer_core(payload)


# HTTP server, auth, /healthz, /assistant/thread and main() from the CK
# runtime are intentionally omitted here: cena_cloud.py owns the HTTP layer
# and calls _answer(payload) directly. Everything above is the verbatim
# supervisor answer logic with only import paths rewritten for the cloud.
