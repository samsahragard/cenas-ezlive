...
# Vendored package keeps CENA_METHODS.md inside cena_engine/assistant_context/ so
# the engine is self-contained on the cloud (runtime used parents[1] = app/; here
# parents[0] = cena_engine/). Load is fault-tolerant: a missing file -> empty
# playbook, never an error.
_METHODS_PATH = Path(__file__).resolve().parents[0] / "assistant_context" / "CENA_METHODS.md"
...
def _default_llm() -> Callable[..., str]:
    from cena_engine.cena_llm import get_default_llm

    return get_default_llm()


def _default_executor() -> Callable[[str], dict]:
    from cena_engine.cena_sql_executor import run_readonly_sql

    return run_readonly_sql


def _default_validate() -> Callable[[str], tuple[bool, str]]:
    from cena_engine.cena_sql_validator import validate_sql
...
def _default_schema_context() -> str:
    try:
        from cena_engine.cena_sql_schema import get_schema_context
...
def _default_memory():
    from cena_engine import cena_memory
...
def _executor_error_types() -> tuple:
    try:
        from cena_engine.cena_sql_executor import CenaSqlError
...
def _llm_error_types() -> tuple:
    try:
        from cena_engine.cena_llm import CenaLlmError
