...
_ANALYTICS_MODULE = "cena_engine.cena_sql_analytics"
_DB_CATALOG_MODULE = "cena_engine.cena_db_catalog"
...
def _snapshot_dir(override: str | None = None) -> Path:
    # Cloud override (additive): CENA_L3_SNAPSHOT_DIR, when set and no explicit
    # `override` arg is given, IS the snapshot directory verbatim (no "/snapshots"
    # suffix). The cena-cloud receiver lands appdb.sqlite / toast.sqlite / ... /
    # cena_analytics.db FLAT in /var/data/cena-db, so point this at that dir.
    # Default behavior (local runtime, no env) is unchanged: <DATA_DIR>/snapshots.
    if override is None:
        direct = os.environ.get("CENA_L3_SNAPSHOT_DIR")
        if direct:
            return Path(direct)
