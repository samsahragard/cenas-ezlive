...
_CURATED_TABLES: dict[str, tuple[str, ...]] = {
    # appdb is now the LIVE prod mirror (2026-06-10), so the vendor / produce /
    # kitchen-prep / fresh-food tables that were empty in the old dev copy are
    # now populated and exposed for reasoning (Sam ask 2026-06-10).
    "appdb": ("orders", "drivers", "delivery_request", "ezcater_known_driver",
              "kitchen_prep_item", "recipes", "vendor_recent_orders",
              "produce_price_snapshot", "kitchen_prep_entry", "fresh_food_order"),
...
_EXPLICIT_EXCLUDED_COLUMNS: dict[str, frozenset[str]] = {
    ...
    # vendor_recent_orders is parsed from inbound vendor emails: drop the raw
    # email plumbing/PII (sender address, raw body, subject, message-id) and the
    # counterparty name. Analytic columns (vendor, store_scope, placed_at,
    # total_cents, status, items_json) stay.
    "appdb.vendor_recent_orders": frozenset({
        "from_addr", "raw_body", "subject", "source_email_mid",
        "customer_or_caterer"}),
...
_STATIC_COLUMNS: dict[str, tuple[str, ...]] = {
    ...
    "appdb.vendor_recent_orders": (
        "id", "vendor", "store_scope", "order_number", "customer_or_caterer",
        "placed_at", "total_cents", "status", "items_json", "tracking_links_json",
        "source_email_mid", "subject", "from_addr", "raw_body", "parse_status",
        "created_at"),
    "appdb.produce_price_snapshot": (
        "id", "created_at", "snapshot_date", "vendor", "canonical_name",
        "canonical_size", "price", "raw_item_name", "parsed_at", "date_range"),
    "appdb.kitchen_prep_entry": (
        "id", "created_at", "updated_at", "entry_date", "store_scope",
        "prep_item_id", "selected", "on_hand", "assignee_name", "status",
        "batch_size", "notes", "locked", "author_id", "prep_qty", "helper_names",
        "completed_by_name", "completed_at"),
    "appdb.fresh_food_order": (
        "id", "placed_at", "order_date", "store_scope", "placed_by_user_id",
        "placed_by_name", "status", "fulfilled_at", "fulfilled_by_user_id",
        "fulfilled_by_name", "sent_date"),
    "toast.time_entry": (
...
_TABLE_NOTES: dict[str, str] = {
    ...
    "appdb.orders":
        "LIVE prod mirror (~855 rows, created_at from 2026-05-07). Catering/corporate "
        "orders + live ops fields. assigned_driver_id->drivers; approved_by_user_id/"
        "pay_verified_by->users. ordersdc.dm_order = deeper sales cube.",
    "appdb.vendor_recent_orders":
        "Vendor purchase orders from supplier emails (44 rows). vendor=supplier, "
        "store_scope, total_cents=our spend, items_json=lines. No internal assignee "
        "(by store only).",
    "appdb.produce_price_snapshot":
        "Produce vendor price list, auto-scraped (532 rows). vendor, canonical_name/"
        "size=item, price=unit cost. No person assigned.",
    "appdb.kitchen_prep_entry":
        "Daily prep assignments (14 rows). WHO makes what: assignee_name, helper_names, "
        "completed_by_name, author_id->users. prep_item_id->kitchen_prep_item; store_scope.",
    "appdb.fresh_food_order":
        "FOH fresh-food orders, EMPTY (0 rows) but wired: placed_by_*/fulfilled_by_* "
        "identify staff, store_scope. Say 'none yet' until data appears.",
    "appdb.drivers":
    ...
    "appdb.kitchen_prep_item": "prep item catalog (47 rows), store_scope scoping. recipe_id->recipes.",
    "appdb.recipes": "recipe book (45 rows): code, category, prep_time, instructions (EN+ES). No per-recipe author (bulk-loaded).",
    "toast.time_entry":
...
def _analytics_doc() -> str:
    ...
    try:  # pragma: no cover - exercised via sys.modules injection in tests
        ...
        mod = importlib.import_module("cena_engine.cena_sql_analytics")
...
def _render_context(ordered: dict[str, tuple[str, ...]]) -> str:
    ...
    parts.append(
        ...
        "- NET-SALES $ BOUNDARY: caterer_total_due is finalized ONLY through "
        "2026-05-09 (ezCater settlement lag ~5 weeks); it is NULL for every later "
        "order. Order COUNT and COVERS (headcount) exist for ALL dates incl. the "
        "current month. For a sales question after 2026-05-09, answer the "
        "count/covers you have, say finalized dollars run through May 9, and offer "
        "the most recent complete week as context - do NOT call the whole question "
        "'unavailable'. A store with zero orders on a date is a real ZERO answer, "
        "distinct from 'dollars not yet settled'.\n"
