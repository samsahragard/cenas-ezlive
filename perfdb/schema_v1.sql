...
-- Phase 5.1 ranking history (Sam #3009/#3014: no-fake held_days). One row per
-- (snapshot_date, period, metric, cohort_key, employee), written each CK-external
-- run. held_days is computed from REAL consecutive snapshots only -- never faked;
-- until snapshots accrue, the UI shows held_days = "pending". CK-local (history);
-- the app receives only the sanitized CURRENT rank output. No sales-$ here -- only
-- the allowed metric value (effective_hourly $/hr or tip_percent ratio) + rank.
CREATE TABLE IF NOT EXISTS rank_snapshot (
  id                INTEGER PRIMARY KEY AUTOINCREMENT,
  snapshot_date     TEXT NOT NULL,           -- ISO date the rank was computed
  period            TEXT NOT NULL,           -- today | week | month | last30
  metric            TEXT NOT NULL,           -- effective_hourly | tip_percent | combined
  cohort_key        TEXT NOT NULL,           -- e.g. store:copperfield|role:tipped
  cohort_size       INTEGER NOT NULL,        -- qualifying peers (min-cohort gate input)
  cena_employee_id  INTEGER NOT NULL,
  store_key         TEXT NOT NULL,
  rank              INTEGER,                 -- NULL when cohort-too-small / unqualified
  pct_rank          REAL,                    -- percentile rank within cohort (0..1)
  value_metric      REAL,                    -- the allowed metric value (NOT sales-$)
  qualified         INTEGER NOT NULL DEFAULT 0,
  computed_at       TEXT,
  UNIQUE (snapshot_date, period, metric, cohort_key, cena_employee_id)
);
CREATE INDEX IF NOT EXISTS idx_rank_snapshot_emp ON rank_snapshot (cena_employee_id, period, metric);
